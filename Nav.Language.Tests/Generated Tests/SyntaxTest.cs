...
//==================================================
// HINWEIS: Diese Datei wurde am 24.01.2024 08:22:10
...
namespace Nav.Language.Tests {
	...
	public class SyntaxTests {
		...
		public void TestConcatGoToEdgeSyntax() {
			var concatGoToEdgeSyntax=Syntax.ParseConcatGoToEdge(SampleSyntax.Of<ConcatGoToEdgeSyntax>());
			
			foreach(var diagnostic in concatGoToEdgeSyntax.SyntaxTree.Diagnostics) {
				Assert.Fail("Die Beispiels-Syntax führt zu Syntaxfehlern:\r\n{0}", diagnostic);
			}
			Assert.That(concatGoToEdgeSyntax.SyntaxTree.Diagnostics.Length, Is.EqualTo(0));
			foreach (var token in concatGoToEdgeSyntax.ChildTokens()) {
		        Assert.That(token.IsMissing, Is.False, "Ein Token ist als 'fehlend' gekennzeichnet:\r\n{0}", token);
		    }
		}
		...
		public void TestConcatModalEdgeSyntax() {
			var concatModalEdgeSyntax=Syntax.ParseConcatModalEdge(SampleSyntax.Of<ConcatModalEdgeSyntax>());
			
			foreach(var diagnostic in concatModalEdgeSyntax.SyntaxTree.Diagnostics) {
				Assert.Fail("Die Beispiels-Syntax führt zu Syntaxfehlern:\r\n{0}", diagnostic);
			}
			Assert.That(concatModalEdgeSyntax.SyntaxTree.Diagnostics.Length, Is.EqualTo(0));
			foreach (var token in concatModalEdgeSyntax.ChildTokens()) {
		        Assert.That(token.IsMissing, Is.False, "Ein Token ist als 'fehlend' gekennzeichnet:\r\n{0}", token);
		    }
		}
		...
		[Test]
		[Description("Syntax: 'Node o-^ Target'\r\n")]
		public void TestConcatTransitionSyntax() {
			var concatTransitionSyntax=Syntax.ParseConcatTransition(SampleSyntax.Of<ConcatTransitionSyntax>());
			
			foreach(var diagnostic in concatTransitionSyntax.SyntaxTree.Diagnostics) {
				Assert.Fail("Die Beispiels-Syntax führt zu Syntaxfehlern:\r\n{0}", diagnostic);
			}
			Assert.That(concatTransitionSyntax.SyntaxTree.Diagnostics.Length, Is.EqualTo(0));
			foreach (var token in concatTransitionSyntax.ChildTokens()) {
		        Assert.That(token.IsMissing, Is.False, "Ein Token ist als 'fehlend' gekennzeichnet:\r\n{0}", token);
		    }
		}
