...
//==================================================
// HINWEIS: Diese Datei wurde am 24.01.2024 08:20:03
...
namespace Nav.Language.Tests {
	...
	public class TokenPropertyNameTests {
		...
		public void TestConcatGoToEdgeSyntax() {
			...
			var tokenProps = typeof(ConcatGoToEdgeSyntax).GetProperties(BindingFlags.Public | BindingFlags.Instance)
                             .Where(m => m.PropertyType ==typeof(SyntaxToken)).ToList();	
			if(!tokenProps.Any()) {
				return;
			}		
			var syntax=Syntax.ParseConcatGoToEdge(SampleSyntax.Of<ConcatGoToEdgeSyntax>());
			foreach (var prop in tokenProps) {    
				if(Attribute.IsDefined(prop, typeof(SuppressCodeSanityCheckAttribute))) {
					continue;
				}
		...
		public void TestConcatModalEdgeSyntax() {
			...
			var tokenProps = typeof(ConcatModalEdgeSyntax).GetProperties(BindingFlags.Public | BindingFlags.Instance)
                             .Where(m => m.PropertyType ==typeof(SyntaxToken)).ToList();	
			if(!tokenProps.Any()) {
				return;
			}		
			var syntax=Syntax.ParseConcatModalEdge(SampleSyntax.Of<ConcatModalEdgeSyntax>());
			foreach (var prop in tokenProps) {    
				if(Attribute.IsDefined(prop, typeof(SuppressCodeSanityCheckAttribute))) {
					continue;
				}
		...
		public void TestConcatTransitionSyntax() {
			...
			var tokenProps = typeof(ConcatTransitionSyntax).GetProperties(BindingFlags.Public | BindingFlags.Instance)
                             .Where(m => m.PropertyType ==typeof(SyntaxToken)).ToList();	
			if(!tokenProps.Any()) {
				return;
			}		
			var syntax=Syntax.ParseConcatTransition(SampleSyntax.Of<ConcatTransitionSyntax>());
			foreach (var prop in tokenProps) {    
				if(Attribute.IsDefined(prop, typeof(SuppressCodeSanityCheckAttribute))) {
					continue;
				}
